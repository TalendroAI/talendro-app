...
const C = { blue: "#2F6DF6", aqua: "#00C4CC", purple: "#8B5CF6" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0a1628 0%, #1a3a5c 50%, #0d2137 100%)", display: "flex", flexDirection: "column", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ height: 4, background: "rgba(255,255,255,0.08)" }}>
        <div style={{ height: "100%", width: `${pct}%`, background: `linear-gradient(90deg, ${C.purple}, ${C.aqua})`, transition: "width 0.4s ease" }} />
      </div>
      <main style={{ flex: 1, padding: "48px 48px 80px", display: "flex", flexDirection: "column", alignItems: "center" }}>
        <div style={{ maxWidth: 720, width: "100%" }}>
          <div style={{ marginBottom: 36, textAlign: "center" }}>
            <div style={{ display: "inline-block", padding: "5px 16px", borderRadius: 20, background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.3)", marginBottom: 14 }}>
...
function Input({ label, value, onChange, placeholder, type = "text", hint, half }) {
  return (
    <div style={{ marginBottom: 18, gridColumn: half ? undefined : "1 / -1" }}>
      <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.6)", marginBottom: 7, textTransform: "uppercase", letterSpacing: 0.7 }}>{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        style={{ width: "100%", padding: "12px 16px", background: "rgba(255,255,255,0.06)", border: "1.5px solid rgba(255,255,255,0.12)", borderRadius: 10, fontSize: 15, color: "#fff", fontFamily: "'Inter', sans-serif", outline: "none", boxSizing: "border-box" }}
        onFocus={e => e.target.style.borderColor = C.purple}
        onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.12)"} />
      {hint && <p style={{ margin: "5px 0 0", fontSize: 12, color: "rgba(255,255,255,0.35)" }}>{hint}</p>}
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      ...
      <select value={value} onChange={e => onChange(e.target.value)}
        style={{ width: "100%", padding: "12px 16px", background: "rgba(255,255,255,0.06)", border: "1.5px solid rgba(255,255,255,0.12)", borderRadius: 10, fontSize: 15, color: value ? "#fff" : "rgba(255,255,255,0.35)", fontFamily: "'Inter', sans-serif", outline: "none" }}>
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      ...
      <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows}
        style={{ width: "100%", padding: "12px 16px", background: "rgba(255,255,255,0.06)", border: "1.5px solid rgba(255,255,255,0.12)", borderRadius: 10, fontSize: 15, color: "#fff", fontFamily: "'Inter', sans-serif", outline: "none", boxSizing: "border-box", resize: "vertical" }}
        onFocus={e => e.target.style.borderColor = C.purple}
        onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.12)"} />
      {hint && <p style={{ margin: "5px 0 0", fontSize: 12, color: "rgba(255,255,255,0.35)" }}>{hint}</p>}
    </div>
...
function NavButtons({ onBack, onNext, nextLabel = "Continue →", disabled = false }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 40 }}>
      {onBack ? <button onClick={onBack} style={{ padding: "12px 28px", border: "1.5px solid rgba(255,255,255,0.2)", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", background: "transparent", color: "rgba(255,255,255,0.7)", fontFamily: "'Inter', sans-serif" }}>← Back</button> : <div />}
      <button onClick={onNext} disabled={disabled}
        style={{ padding: "14px 48px", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer", background: disabled ? "rgba(255,255,255,0.1)" : C.purple, color: disabled ? "rgba(255,255,255,0.3)" : "#fff", fontFamily: "'Montserrat', sans-serif", boxShadow: disabled ? "none" : `0 4px 20px ${C.purple}50`, transition: "all 0.2s" }}>
        {nextLabel}
      </button>
    </div>
  );
...
export default function ResumeCreate() {
  ...
  if (step === 2) return (
    <PageShell step={2} totalSteps={4} title="Your Work History" subtitle="Add your most recent positions first. Include part-time, contract, and freelance work.">
      <label style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer", marginBottom: 28, padding: "14px 20px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10 }}>
        <input type="checkbox" checked={noExperience} onChange={e => setNoExperience(e.target.checked)} style={{ width: 18, height: 18, accentColor: C.purple, flexShrink: 0 }} />
        <div>
          <span style={{ fontSize: 15, fontWeight: 600, color: "#fff" }}>I have no work experience</span>
          <p style={{ margin: "2px 0 0", fontSize: 13, color: "rgba(255,255,255,0.4)" }}>New graduates, career changers, or first-time job seekers — AI will highlight your education and skills instead.</p>
        </div>
      </label>
      ...
      {!noExperience && (
        <>
          {jobs.map((job, i) => (
            <div key={i} style={{ padding: "24px 28px", background: "rgba(255,255,255,0.03)", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 16, marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <h4 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#fff", fontFamily: "'Montserrat', sans-serif" }}>Position {i + 1}</h4>
                {jobs.length > 1 && <button onClick={() => removeJob(i)} style={{ padding: "5px 14px", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 6, fontSize: 12, cursor: "pointer", background: "rgba(239,68,68,0.08)", color: "#FCA5A5", fontFamily: "'Inter', sans-serif" }}>Remove</button>}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, cursor: "pointer" }}>
                <input type="checkbox" checked={job.current} onChange={e => updateJob(i, "current", e.target.checked)} style={{ width: 16, height: 16, accentColor: C.purple }} />
                <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)" }}>I currently work here</span>
              </label>
              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.6)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.7 }}>Key Achievements (up to 3 bullet points)</label>
                <p style={{ margin: "0 0 10px", fontSize: 12, color: "rgba(255,255,255,0.35)" }}>Start with action verbs: "Led...", "Built...", "Increased...", "Managed..." — AI will polish these.</p>
                {job.bullets.map((b, bi) => (
                  <input key={bi} type="text" value={b} onChange={e => updateBullet(i, bi, e.target.value)}
                    placeholder={`Achievement ${bi + 1}...`}
                    style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,0.05)", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 8, fontSize: 14, color: "#fff", fontFamily: "'Inter', sans-serif", outline: "none", boxSizing: "border-box", marginBottom: 8 }} />
                ))}
              </div>
            </div>
          ))}
          <button onClick={addJob} style={{ width: "100%", padding: "14px", border: "1.5px dashed rgba(255,255,255,0.2)", borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: "pointer", background: "transparent", color: "rgba(255,255,255,0.5)", fontFamily: "'Inter', sans-serif", marginBottom: 8 }}>
  ...
  if (step === 3) return (
    <PageShell step={3} totalSteps={4} title="Education & Skills" subtitle="Add your highest education and the skills that make you stand out.">
      <h3 style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: "'Montserrat', sans-serif", marginBottom: 16 }}>Education</h3>
      {education.map((edu, i) => (
        <div key={i} style={{ padding: "20px 24px", background: "rgba(255,255,255,0.03)", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 14, marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>Education {i + 1}</span>
            {education.length > 1 && <button onClick={() => removeEdu(i)} style={{ padding: "4px 12px", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 6, fontSize: 12, cursor: "pointer", background: "rgba(239,68,68,0.08)", color: "#FCA5A5", fontFamily: "'Inter', sans-serif" }}>Remove</button>}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
      ...
      <button onClick={addEdu} style={{ width: "100%", padding: "12px", border: "1.5px dashed rgba(255,255,255,0.15)", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", background: "transparent", color: "rgba(255,255,255,0.4)", fontFamily: "'Inter', sans-serif", marginBottom: 32 }}>
        + Add Another Degree or Certification
      </button>

      <h3 style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: "'Montserrat', sans-serif", marginBottom: 16 }}>Skills & Expertise</h3>
  ...
  if (step === 4) return (
    <PageShell step={4} totalSteps={4} title="Your Career Goals" subtitle="Help AI tailor your resume to the right opportunities from day one.">
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer", marginTop: 8, marginBottom: 32, padding: "14px 20px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10 }}>
        <input type="checkbox" checked={goals.openToRelocation} onChange={e => setGoals(p => ({ ...p, openToRelocation: e.target.checked }))} style={{ width: 18, height: 18, accentColor: C.purple, flexShrink: 0 }} />
        <span style={{ fontSize: 15, color: "rgba(255,255,255,0.8)" }}>I am open to relocation</span>
      </label>

      <div style={{ padding: "20px 24px", background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.25)", borderRadius: 14, marginBottom: 8 }}>
        <h4 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#fff", fontFamily: "'Montserrat', sans-serif", marginBottom: 8 }}>✨ What happens next</h4>
