...
export default function InterviewCoach() {
  ...
  const checkForPausedSessions = async (): Promise<PausedSession | null> => {
    ...
    try {
      const _gpsTok = localStorage.getItem('authToken') || localStorage.getItem('talendro_token') || '';
      const _gpsRes = await fetch('/api/interview/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${_gpsTok}` },
        body: JSON.stringify({ action: 'get_paused', email: derivedEmail }),
      });
      const _gpsData = await _gpsRes.json();
      if (!_gpsRes.ok) {
        console.error('Error checking paused sessions:', _gpsData?.error);
        return null;
      }
      const sessions = (_gpsData?.sessions as PausedSession[]) ?? [];
  ...
  const proceedWithSaveDocuments = async () => {
    ...
    if (isPaymentVerified && activeSessionType === 'quick_prep' && sessionId) {
      ...
      try {
        const _qpTok = localStorage.getItem('authToken') || localStorage.getItem('talendro_token') || '';
        const _qpRes = await fetch('/api/interview/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${_qpTok}` },
          body: JSON.stringify({
        ...
        const _qpData = await _qpRes.json();
        if (!_qpRes.ok) {
          throw new Error(_qpData?.error || 'Failed to generate content');
        }
        if (_qpData?.message) {
          setQuickPrepContent(_qpData.message);
        } else {
      ...
      try {
        // Generate prep packet using the Quick Prep prompt
        const _ppTok = localStorage.getItem('authToken') || localStorage.getItem('talendro_token') || '';
        const _ppRes = await fetch('/api/interview/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${_ppTok}` },
          body: JSON.stringify({
            session_type: 'quick_prep',
            session_id: sessionId,
            documents,
            is_initial: true,
            generate_prep_only: true,
          }),
        });
        if (!_ppRes.ok) {
          console.error('Failed to generate prep packet for Audio Mock:', await _ppRes.json());
  ...
  const handleAbandonAndStartNew = async (abandonedSessionId) => {
    ...
    try {
      // Abandon session via Express API
      const _abTok = localStorage.getItem('authToken') || localStorage.getItem('talendro_token') || '';
      const _abRes = await fetch('/api/interview/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${_abTok}` },
        body: JSON.stringify({ action: 'abandon', sessionId: abandonedSessionId }),
      });
      const _abData = await _abRes.json();
      if (!_abRes.ok || !_abData?.ok) {
        console.error('Error abandoning session:', _abData);
        toast({
          variant: 'destructive',
          title: 'Error',
