...
import { generateSearchProfileAndFindJobs, validateBooleanString } from '../../utils/booleanSearchGenerator';
...
const Onb4 = () => {
  ...
  // Employment history duration tracking
  const [employmentYears, setEmploymentYears] = useState(0);
  const REQUIRED_EMPLOYMENT_YEARS = 10;
  ...
  // Map degree names from parsed resume to our dropdown values
  const mapDegreeType = (degree) => {
    const lower = (degree || '').toLowerCase();
    
    // Check for full degree names first
    if (lower.includes('bachelor')) return 'bachelor';
    if (lower.includes('master')) return 'master';
    if (lower.includes('doctorate') || lower.includes('doctoral') || lower.includes('phd') || lower.includes('ph.d')) return 'doctoral';
    if (lower.includes('associate')) return 'associate';
    
    // Then check for abbreviations
    if (lower.includes('bs') || lower.includes('ba') || lower.includes('b.s') || lower.includes('b.a')) return 'bachelor';
    if (lower.includes('ms') || lower.includes('ma') || lower.includes('mba') || lower.includes('m.s') || lower.includes('m.a')) return 'master';
    if (lower.includes('high school') || lower.includes('diploma') || lower.includes('ged')) return 'high_school';
    if (lower.includes('jd') || lower.includes('md') || lower.includes('dds') || lower.includes('j.d') || lower.includes('m.d')) return 'professional';
    if (lower.includes('certificate')) return 'certificate';
    if (lower.includes('trade') || lower.includes('vocational')) return 'trade';
  ...
  const formatDateForInput = (dateString) => {
    ...
    try {
      // If it's already in YYYY-MM or YYYY-MM-DD format
  ...
  useEffect(() => {
    ...
    if (resumeData?.profileDraft) {
      // Parse Education from profileDraft.education
      if (resumeData.profileDraft.education && Array.isArray(resumeData.profileDraft.education) && resumeData.profileDraft.education.length > 0) {
        const educationData = resumeData.profileDraft.education.map((edu, index) => ({
          id: index + 1,
          institution: edu.institutionName || '',
          degreeType: mapDegreeType(edu.highestDegree || ''),
          fieldOfStudy: edu.majorFieldOfStudy || '',
          graduationDate: formatDateForInput(edu.graduationDate) || '',
          gpa: edu.gpa || '',
          honors: '',
          additionalDetails: '',
          stillEnrolled: edu.current || false
        }));
        
        // Only set if education is currently empty
        if (education.length === 1 && !education[0].institution) {
    ...
    if (prefill?.step4) {
      ...
      // Populate education if available (from profileDraft.education)
      // Note: This section is already handled above in profileDraft.education parsing
      // This section is kept for backward compatibility with step4.education format
      if (step4.education && step4.education.length > 0 && education.length === 1 && !education[0].institution) {
        const educationData = step4.education.map((edu, index) => ({
          ...
          degreeType: mapDegreeType(edu.highestDegree || edu.degree || edu.studyType || ''),
          fieldOfStudy: edu.majorFieldOfStudy || edu.area || edu.major || '',
          graduationDate: formatDateForInput(edu.graduationDate || edu.endDate) || '',
  ...
  // Calculate total years of employment history
  const calculateEmploymentYears = (jobs) => {
    let totalMonths = 0;
    ...
    jobs.forEach(job => {
      ...
      if (job.current || !job.end) {
        endDate = new Date(); // Current date if still employed
      } else {
  ...
  // Recalculate employment years whenever jobs change
  useEffect(() => {
    const years = calculateEmploymentYears(employment);
    setEmploymentYears(parseFloat(years));
    
    // Check if all current jobs are filled
    const allFilled = employment.every(job => 
      job.company && job.title && job.start
    );
    
    // AUTO-ADD: If under 10 years and all jobs are filled
    if (parseFloat(years) < REQUIRED_EMPLOYMENT_YEARS && allFilled && employment.length > 0) {
      const lastJob = employment[employment.length - 1];
      if (lastJob && !lastJob.current) {
        // Don't auto-add more than 15 jobs
        if (employment.length < 15) {
          addEmployment();
        }
      }
    }
    
    // AUTO-REMOVE: If 10+ years reached, remove empty auto-added jobs
    if (parseFloat(years) >= REQUIRED_EMPLOYMENT_YEARS && employment.length > 1) {
      // Find empty jobs (likely auto-added)
      const emptyJobs = employment.filter(job => 
        !job.company && !job.title && !job.start
      );
      
      // Remove empty jobs, but keep at least 1 job
      if (emptyJobs.length > 0 && employment.length - emptyJobs.length >= 1) {
        setEmployment(prev => 
          prev.filter(job => 
            job.company || job.title || job.start
          )
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate criminal history details if "Yes" was selected
    if (formData.criminalHistory === 'yes' && !formData.criminalHistoryDetails?.trim()) {
      alert('Please provide details about your criminal history before continuing.');
      document.getElementById('criminalHistoryDetails')?.focus();
      return;
    }
    
    // Validate employment history
    if (employmentYears < REQUIRED_EMPLOYMENT_YEARS) {
      alert(`You must provide ${REQUIRED_EMPLOYMENT_YEARS} years of employment history. You currently have ${employmentYears.toFixed(2)} years. Please add additional employers.`);
      document.getElementById('employmentHistory')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
    
    // Data is already auto-saved via useAutoSave
    
    // Set loading state
    setIsGeneratingSearch(true);
    const submitButton = e.target.querySelector('button[type="submit"]');
    const originalText = submitButton?.textContent || 'Continue';
    
    // Initialize profileData outside try block so it's accessible in catch
    let profileData = null;
    
    try {
      // 1. Collect all data from Steps 1-4
      const step1Data = loadSavedData('onboarding_step1', {});
      const step3Data = loadSavedData('onboarding_step3', {});
      const step4FormData = loadSavedData('onboarding_step4_form', {});
      const step4Employment = loadSavedData('onboarding_step4_employment', []);
      const step4Education = loadSavedData('onboarding_step4_education', []);
      const step4Licenses = loadSavedData('onboarding_step4_licenses', []);
      const step4Certifications = loadSavedData('onboarding_step4_certifications', []);
      
      // Update button text
      if (submitButton) {
        submitButton.disabled = true;
        submitButton.innerHTML = '<span className="inline-block animate-spin mr-2">⏳</span> Processing Profile...';
      }
      
      // 2. Combine all profile data for Boolean generation
      profileData = {
        personalInfo: {
          fullLegalName: step3Data.fullLegalName || `${step1Data.firstName || ''} ${step1Data.lastName || ''}`.trim(),
          city: step3Data.city || '',
          state: step3Data.state || '',
          email: step3Data.email || step1Data.email || '',
          phone: step3Data.phone || step1Data.phone || ''
        },
        professionalInfo: {
          desiredSalary: step4FormData.desiredSalary || '',
          availableStartDate: step4FormData.availableStartDate || '',
          workArrangement: step4FormData.workArrangement || [],
          willingToRelocate: step4FormData.willingToRelocate || '',
          travelPercentage: step4FormData.travelPercentage || '',
          hasSecurityClearance: step4FormData.hasSecurityClearance || '',
          clearanceLevel: step4FormData.clearanceLevel || ''
        },
        workHistory: step4Employment || [],
        education: step4Education || [],
        skills: [], // Will be extracted from resume data if available
        licenses: step4Licenses || [],
        certifications: step4Certifications || []
      };
      
      // Try to get skills from resume data
      const resumeData = JSON.parse(localStorage.getItem('resumeData') || 'null');
      if (resumeData?.profileDraft?.skills) {
        profileData.skills = resumeData.profileDraft.skills.map(skill => 
          typeof skill === 'string' ? skill : skill.name || skill.skill
        );
      }
      
      // Update button text for search generation
      if (submitButton) {
        submitButton.innerHTML = '<span className="inline-block animate-spin mr-2">🔍</span> Generating Search Strategy...';
      }
      
      // 3. Generate Boolean search string AND execute job search (CRITICAL)
      const progressCallback = (message) => {
        if (submitButton) {
          if (message.includes('Analyzing')) {
            submitButton.innerHTML = '<span className="inline-block animate-spin mr-2">⏳</span> Analyzing Profile...';
          } else if (message.includes('Searching')) {
            submitButton.innerHTML = '<span className="inline-block animate-spin mr-2">🔍</span> Finding Jobs...';
          } else if (message.includes('Saving')) {
            submitButton.innerHTML = '<span className="inline-block animate-spin mr-2">💾</span> Saving Results...';
          }
        }
      };
      
      const searchResults = await generateSearchProfileAndFindJobs(profileData, progressCallback);
      
      // 4. Validate the search string
      const validation = validateBooleanString(searchResults.searchString);
      if (!validation.valid) {
        throw new Error(validation.error);
      }
      
      console.log('✓ Search Profile Generated:', searchResults.searchString);
      console.log(`✓ Found ${searchResults.totalJobsDiscovered} jobs, ${searchResults.jobsMatched} matched 75%+`);
      ...
      console.error('=== STEP 4 FORM SUBMISSION ERROR ===');
      console.error('Error:', error);
      console.error('Error message:', error.message);
      if (profileData) {
        console.error('Profile data being sent:', JSON.stringify(profileData, null, 2));
      } else {
        console.error('Profile data was not assembled (error occurred before data collection)');
      }
      
      // Detect specific error types
      let userMessage = 'We encountered an issue analyzing your profile.\n\n';
      
      if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
        userMessage += 'Network Error: Cannot reach the AI service. Check your internet connection.\n\n';
      } else if (error.message.includes('401')) {
        userMessage += 'Authentication Error: API key is invalid or missing.\n\n';
      } else if (error.message.includes('429')) {
        userMessage += 'Rate Limit Error: Too many requests. Please wait a moment and try again.\n\n';
      } else {
        userMessage += `Error: ${error.message}\n\n`;
      }
      
      userMessage += 'You can still proceed - we\'ll generate your search profile after signup.';
      
      // Show detailed error to user (for debugging)
      alert(`Error analyzing profile:\n\n${error.message}\n\nCheck browser console (F12) for details.\n\nYou can still proceed - we'll generate your search profile after signup.`);
      ...
      // Still allow navigation (we can generate search string later if needed)
      // Uncomment below if you want to let them proceed despite error:
      // navigate('/app/onboarding/step-5');
    }
  };
    ...
    <section>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="card">
          ...
          {/* Conditional: Criminal History Details */}
          {formData.criminalHistory === 'yes' && (
            <div className="mt-4 p-4 bg-yellow-50 border-2 border-yellow-300 rounded-lg">
              <label className="block body mb-2 font-semibold text-yellow-900">
                Please provide details about your criminal history <span className="text-red-600">*</span>
              </label>
              <textarea
                ...
                required
                rows="4"
                placeholder="Include: nature of offense, date, location, outcome, and current status. Be honest and thorough - this information is used for background check disclosure."
                className="w-full px-3 py-2 border-2 border-red-300 bg-red-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-talAqua transition resize-vertical"
              />
              <p className="text-xs text-yellow-800 mt-2">
                ⚠️ Note: A criminal history does not automatically disqualify you from employment. Many employers evaluate applications on a case-by-case basis.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="card">
          <div className="mb-4 pb-2 border-b border-gray-200">
            <div className="flex justify-between items-center mb-2">
              <h2 className="h3">Employment History (10 Years Required)</h2>
              <div className={`text-sm font-semibold px-4 py-2 rounded-lg ${
                employmentYears >= REQUIRED_EMPLOYMENT_YEARS 
                  ? 'bg-green-100 text-green-800' 
                  : 'bg-yellow-100 text-yellow-800'
              }`}>
                {employmentYears.toFixed(2)} / {REQUIRED_EMPLOYMENT_YEARS}.00 years
                {employmentYears < REQUIRED_EMPLOYMENT_YEARS && (
                  <span className="ml-2">• {(REQUIRED_EMPLOYMENT_YEARS - employmentYears).toFixed(2)} years remaining</span>
                )}
              </div>
            </div>
            
            {employmentYears < REQUIRED_EMPLOYMENT_YEARS && employment.length > 0 && (
              <div className="mb-4 p-3 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                <p className="text-sm text-yellow-800">
                  ⚠️ You need to provide <strong>{(REQUIRED_EMPLOYMENT_YEARS - employmentYears).toFixed(2)} more years</strong> of employment history. Add additional employers below.
                </p>
              </div>
            )}
            
            <div className="info-box mb-4">
              <p><strong>Note:</strong> If currently employed, check "Current" and leave end date blank. If no longer employed, select an end date and leave "Current" unchecked.</p>
            </div>
          </div>
          ...
          {/* Always show "Add Another Employer" button */}
          <button 
            type="button" 
            onClick={addEmployment}
            className="mt-4 px-4 py-2 bg-white text-talBlue border-2 border-talBlue rounded-lg font-medium hover:bg-blue-50 transition"
          >
            + Add Another Employer {employmentYears >= REQUIRED_EMPLOYMENT_YEARS ? '(Optional)' : ''}
          </button>
        </div>
