...
function FormField({ field, value, onChange, error, blankFields = new Set() }) {
  ...
  const inputProps = {
    ...
    required,
    'data-testid': testId,
    className: `w-full h-10 px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${error || blankFields.has(key) ? 'border-red-500 bg-red-50 ring-2 ring-red-200' : ''}`
  }
    ...
    return (
      <div className="space-y-1">
        ...
        <textarea
          ...
          data-testid={testId}
          rows={3}
          className={`w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y ${error || blankFields.has(key) ? 'border-red-500 bg-red-50 ring-2 ring-red-200' : ''}`}
        />
        {error && <p className="text-sm" style={{color: '#dc2626'}}>{error}</p>}
      </div>
    )
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-4">
          <input
            ...
            required={required}
            data-testid={testId}
            className={`flex-1 h-10 px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${error || blankFields.has(key) ? 'border-red-500' : ''}`}
          />
      ...
      <div className="space-y-1">
        ...
        <div className="relative multiselect-container">
          <button
            ...
            className={`w-full h-10 px-3 py-2 bg-white border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-left flex items-center justify-between ${error || blankFields.has(key) ? 'border-red-500' : ''}`}
          >
...
function CollectionItem({ collection, item, index, onChange, onRemove, errors }) {
