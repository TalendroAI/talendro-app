...
function FormField({ field, value, onChange, error }) {
  ...
  const inputProps = {
    ...
    required,
    'data-testid': testId,
    className: `w-full h-10 px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${error ? 'border-red-500 bg-red-50 ring-2 ring-red-200' : ''}`
  }
    ...
    return (
      <div className="space-y-1">
        ...
        <textarea
          ...
          data-testid={testId}
          rows={3}
          className={`w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y ${error ? 'border-red-500 bg-red-50 ring-2 ring-red-200' : ''}`}
        />
        {error && <p className="text-sm text-red-600">{error}</p>}
      </div>
    )
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-4">
          <input
            ...
            required={required}
            data-testid={testId}
            className={`flex-1 h-10 px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${error ? 'border-red-500' : ''}`}
          />
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-red-700">
                <p className="text-red-600 font-semibold">You cannot proceed until all required fields are completed:</p>
                <ul className="list-disc list-inside mt-2">
                  {incompleteFields.map((field, index) => (
                    <li key={index} className="font-medium text-red-600">{field}</li>
                  ))}
                </ul>
              </div>
            </div>
      ...
      {schema.collections && schema.collections.map(collection => (
        <div key={collection.key} className="bg-white p-6 rounded-lg border border-gray-300">
          ...
          {errors[`${collection.key}_coverage`] && (
            <p className="text-sm text-red-600 mt-2">{errors[`${collection.key}_coverage`]}</p>
          )}
        </div>
      ))}
