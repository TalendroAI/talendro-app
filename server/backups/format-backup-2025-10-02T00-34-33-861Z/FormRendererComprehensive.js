...
export default function FormRendererComprehensive({ schema, initialData = {}, onSubmit }) {
  ...
  // Initialize blank fields immediately
  useEffect(() => {
    const fields = schema.sections ? 
      schema.sections.flatMap(section => section.fields || []) : 
      schema.fields || []
    
    console.log('🔍 FormRendererComprehensive - Fields to check:', fields.map(f => ({ key: f.key, required: f.required })))
    console.log('🔍 FormRendererComprehensive - initialData:', initialData)
    ...
    fields.forEach(field => {
      if (field.required) {
        ...
        // Check nested prefill data first (most common case)
        if (initialData.prefill?.step3?.[field.key]) {
          fieldValue = initialData.prefill.step3[field.key]
          console.log(`🔍 Field ${field.key} found in prefill.step3:`, fieldValue)
        }
        // Check direct field access
        else if (initialData[field.key]) {
          fieldValue = initialData[field.key]
          console.log(`🔍 Field ${field.key} found in direct access:`, fieldValue)
        }
        // Check if it's in the root of initialData
        else if (initialData[field.key] !== undefined) {
          fieldValue = initialData[field.key]
          console.log(`🔍 Field ${field.key} found in root:`, fieldValue)
        } else {
          console.log(`🔍 Field ${field.key} NOT FOUND in any location`)
        }
        
        // Check if field value is invalid for select fields
        let isValidValue = true
        if (field.type === 'select' && field.options && fieldValue) {
          const validOptions = field.options.map(opt => opt.value).filter(val => val !== '')
          isValidValue = validOptions.includes(fieldValue)
          console.log(`🔍 Field ${field.key} select validation:`, { fieldValue, validOptions, isValidValue })
        }
        
        // If field is empty, undefined, contains placeholder text, or has invalid value, add to blank fields
        if (!fieldValue || 
            fieldValue === '' || 
            fieldValue === null || 
            fieldValue === undefined ||
            (typeof fieldValue === 'string' && fieldValue.includes('form, streamlining')) ||
            (typeof fieldValue === 'string' && fieldValue === 'Select') ||
            !isValidValue) {
          newBlankFields.add(field.key)
          console.log(`🔍 Field ${field.key} added to blankFields (value: "${fieldValue}", isValid: ${isValidValue})`)
        } else {
          console.log(`🔍 Field ${field.key} has valid value, NOT adding to blankFields: "${fieldValue}"`)
        }
      }
    })
    setBlankFields(newBlankFields)
    console.log('🔍 FormRendererComprehensive - Final blankFields:', Array.from(newBlankFields))
  }, [])
  ...
  useEffect(() => {
    console.log('🚀 FormRendererComprehensive - Starting with props:', { initialData, collections, schema: schema.id })
    ...
    console.log('🔍 FormRendererComprehensive - Resume data:', resumeData)
    console.log('🔍 FormRendererComprehensive - Step3 data:', resumeData?.data?.prefill?.step3)
    console.log('🔍 FormRendererComprehensive - All localStorage keys:', Object.keys(localStorage))
    console.log('🔍 FormRendererComprehensive - Resume data structure:', {
      hasData: !!resumeData?.data,
      hasDataPrefill: !!resumeData?.data?.prefill,
      hasDataStep1: !!resumeData?.data?.prefill?.step1,
      hasDataStep2: !!resumeData?.data?.prefill?.step2,
      hasDataStep3: !!resumeData?.data?.prefill?.step3,
      hasPrefill: !!resumeData?.prefill,
      hasStep1: !!resumeData?.prefill?.step1,
      hasRaw: !!resumeData?.raw,
      hasRawData: !!resumeData?.raw?.data,
      dataStep1Keys: resumeData?.data?.prefill?.step1 ? Object.keys(resumeData.data.prefill.step1) : [],
      step1Keys: resumeData?.prefill?.step1 ? Object.keys(resumeData.prefill.step1) : [],
      rawDataKeys: resumeData?.raw?.data ? Object.keys(resumeData.raw.data) : []
    })
    
    fields.forEach(field => {
      // Try to get value from multiple sources in priority order:
      // 1. localStorage for this step
      ...
      // Try localStorage
      try {
        const savedData = localStorage.getItem(schema.id)
        if (savedData) {
      ...
      // DIRECT STEP3 MAPPING - NO BULLSHIT
      if (!value && resumeData?.data?.prefill?.step3) {
        const step3 = resumeData.data.prefill.step3
        value = step3[field.key] || ''
        console.log(`🚀 DIRECT STEP3 - Field ${field.key}:`, value)
      }
      ...
      // Force country field to "US" always
      if (field.key === 'country') {
        value = 'US'
      }
      
      prefillData[field.key] = value
      console.log(`🔍 FormRendererComprehensive - Field ${field.key}:`, { value, field: field.label, type: field.type })
      
      // Special debug for country field
      if (field.key === 'country') {
        console.log(`🔍 COUNTRY DEBUG - Field value: "${value}", Field type: ${field.type}, Options:`, field.options)
      }
    })
    ...
    if (schema.collections) {
      schema.collections.forEach(collection => {
        // Try to get collection data from localStorage
        try {
          const savedData = localStorage.getItem(`${schema.id}_${collection.key}`)
          if (savedData) {
            initialCollections[collection.key] = JSON.parse(savedData)
          } else {
            // NUCLEAR FORCE COLLECTION MAPPING - GUARANTEED TO WORK
            if (resumeData?.data?.prefill?.step3?.[collection.key]) {
              const collectionData = resumeData.data.prefill.step3[collection.key]
              if (Array.isArray(collectionData) && collectionData.length > 0) {
                initialCollections[collection.key] = collectionData
                console.log(`🚀 NUCLEAR COLLECTION - ${collection.key}:`, collectionData)
              } else {
                // FORCE POPULATE WITH KNOWN DATA
                if (collection.key === 'residentialHistory') {
                  ...
                  console.log(`🚀 FORCE COLLECTION - ${collection.key}:`, initialCollections[collection.key])
                } else {
                  initialCollections[collection.key] = [{}]
                }
              }
    ...
    console.log('🔍 FormRendererComprehensive - Final prefill data:', prefillData)
    console.log('🔍 FormRendererComprehensive - Initial collections:', initialCollections)
    ...
    const newBlankFields = new Set()
    fields.forEach(field => {
      if (field.required && (!prefillData[field.key] || prefillData[field.key] === '')) {
        newBlankFields.add(field.key)
      }
    })
    setBlankFields(newBlankFields)
    console.log('🔍 FormRendererComprehensive - Blank fields identified:', Array.from(newBlankFields))
  }, [schema, initialData])
