...
export default function Onb4() {
  ...
  useEffect(() => {
    ...
    const savedData = localStorage.getItem('step-4-comprehensive')
    if (savedData) {
      try {
        const parsedData = JSON.parse(savedData)
        setPrefillData(parsedData)
        console.log('🔍 Loaded previously saved data for step-4:', parsedData)
      } catch (error) {
        console.log('🔍 Error loading saved data for step-4:', error)
      }
    } else {
      // If no saved data, try to load resume data for parsing
      const resumeData = localStorage.getItem('resumeData')
      if (resumeData) {
        try {
          const parsedResumeData = JSON.parse(resumeData)
          setPrefillData(parsedResumeData)
          console.log('🔍 Loaded resume data for step-4 parsing:', parsedResumeData)
        } catch (error) {
          console.log('🔍 Error loading resume data for step-4:', error)
        }
      }
    }
  ...
  const handleDisclosuresClick = () => {
    ...
    console.log('🔍 Onb4 validation check:', { blankFields })
    ...
    const step4Data = JSON.parse(localStorage.getItem('step-4') || '{}')
    
    // Get all required fields from the schema
    const requiredFields = step4Schema.sections.flatMap(section => 
    ...
    const actuallyBlankFields = requiredFields.filter(field => {
      const fieldValue = step4Data[field.key]
    ...
    console.log('🔍 Onb4 actually blank fields:', actuallyBlankFields.map(f => f.key))
    ...
    console.log('🔍 Step-4 completed via top nav, data already saved to localStorage via auto-save')
    window.location.href = '/app/onboarding/step-5'
  }

  const handleFormSubmit = async (formData) => {
    console.log('🔍 Onb4 handleFormSubmit called with:', formData)
    console.log('🔍 Onb4 handleFormSubmit - formData keys:', Object.keys(formData))
    console.log('🔍 Onb4 handleFormSubmit - formData values:', formData)
    alert('🔍 Onb4 handleFormSubmit called! Check console for details.')
    
    // Save form data to localStorage first
    localStorage.setItem('step-4', JSON.stringify(formData))
    
    // Check if there are any blank required fields using the same logic as handleDisclosuresClick
    const requiredFields = step4Schema.sections.flatMap(section => 
      section.fields?.filter(field => field.required) || []
    )
    ...
    console.log('🔍 Onb4 handleFormSubmit - actually blank fields:', actuallyBlankFields.map(f => f.key))
    
    if (actuallyBlankFields.length > 0) {
      console.log('🔍 Onb4 handleFormSubmit - blocking navigation due to blank fields')
      setShowWarning(true)
      return
    }
    ...
    console.log('🔍 Step-4 completed, data already saved to localStorage via auto-save')
