...
export default function Page(){
  ...
  useEffect(() => {
    try {
      ...
      if (resumeData?.prefill?.step1) {
        ...
        if (!firstName || firstName.trim() === '') newBlankFields.add('firstName')
        if (!lastName || lastName.trim() === '') newBlankFields.add('lastName')
        if (!prefill.email || prefill.email.trim() === '') newBlankFields.add('email')
        if (!prefill.phone || prefill.phone.trim() === '') newBlankFields.add('phone')
  ...
  // Force ALL fields to be marked as blank on mount - this ensures red highlighting
  useEffect(() => {
    setBlankFields(new Set(['firstName', 'lastName', 'email', 'phone', 'password', 'confirmPassword', 'agreeToTerms']))
  }, [])
