...
const Onb3 = () => {
  ...
  useEffect(() => {
    ...
    if (prefill?.step2) {
      ...
      setFormData(prev => ({
        ...prev,
        fullLegalName: step2.fullLegalName || '',
        preferredFirstName: step2.preferredFirstName || '',
        maidenName: step2.maidenName || '',
        previousNames: step2.previousNames || '',
        email: step2.email || '',
        phone: step2.phone || '',
        linkedinUrl: step2.linkedinUrl || '',
        website: step2.website || '',
        streetAddress: step2.streetAddress || '',
        city: step2.city || '',
        state: step2.state || '',  // This will populate the dropdown
        postalCode: step2.postalCode || '',
        county: step2.county || '',
        country: step2.country || 'US',
        emergencyName: step2.emergencyName || '',
        emergencyRelationship: step2.emergencyRelationship || '',
        emergencyPhone: step2.emergencyPhone || '',
        emergencyPhoneAlt: step2.emergencyPhoneAlt || '',
        dlNumber: step2.dlNumber || '',
        dlState: step2.dlState || '',
        dateOfBirth: step2.dateOfBirth || '',
        ssnLast4: step2.ssnLast4 || ''
      }));
      
      // Populate residential history (if your form has a function to add residences)
      if (step2.residentialHistory && step2.residentialHistory.length > 0 && typeof window.addResidence === 'function') {
        step2.residentialHistory.forEach((residence, index) => {
          if (index > 0) {
            window.addResidence(); // Add additional residence fields if needed
          }
          // Populate each residence
          setTimeout(() => {
            const resNum = index + 1;
            const fields = {
              [`residence${resNum}_street`]: residence.streetAddress,
              [`residence${resNum}_city`]: residence.city,
              [`residence${resNum}_state`]: residence.state,
              [`residence${resNum}_zip`]: residence.postalCode,
              [`residence${resNum}_from`]: residence.fromDate,
              [`residence${resNum}_to`]: residence.current ? '' : residence.toDate,
              [`residence${resNum}_current`]: residence.current
            };
