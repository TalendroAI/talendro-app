...
function extractEducationFromProfileDraft(education, fieldKey) {
  ...
  const mapping = {
    institutionName: education.institution || education.institutionName || education.school || '',
    majorFieldOfStudy: education.fieldOfStudy || education.major || education.majorFieldOfStudy || '',
    highestDegree: education.degree || education.degreeType || education.educationLevel || '',
    graduationDate: formatDateForInput(education.endDate || education.graduationDate || education.completionDate),
    attendanceStartDate: formatDateForInput(education.startDate || education.attendanceStartDate),
    gpa: education.gpa || education.grade || '',
    institutionCity: education.location?.city || education.city || '',
    institutionState: education.location?.state || education.state || '',
    institutionAddress: education.location?.address || education.address || '',
    completedAsOfToday: education.completed ? 'yes' : 'no'
  }
...
function extractWorkFromProfileDraft(work, fieldKey) {
  ...
  const mapping = {
    currentJobTitle: work.position || work.jobTitle || work.title || '',
    currentEmployer: work.company || work.employer || work.organization || '',
    fromDate: formatDateForInput(work.startDate || work.fromDate),
    endDate: formatDateForInput(work.endDate || work.toDate)
  }
...
useEffect(() => {
  ...
  // STEP 4: Enhanced hydration using multiple data sources
  const initialForm = {}
  ...
  fields.forEach(field => {
    ...
    // Try step3Data first (direct mapping)
    if (step3Data[fieldKey] !== undefined && step3Data[fieldKey] !== null && step3Data[fieldKey] !== '') {
      value = step3Data[fieldKey]
      console.log(`✅ Direct hit for ${fieldKey}:`, value)
    }
    // Try prefillKeys if available
    else if (field.prefillKeys && field.prefillKeys.length > 0) {
      value = extractPrefillValue(field.prefillKeys, source) || ''
      if (value) {
        console.log(`🔍 PrefillKeys for ${fieldKey}:`, value)
      }
    }
    // Try profileDraft for education fields
    else if (profileDraft.education && Array.isArray(profileDraft.education) && profileDraft.education.length > 0) {
    ...
    // Try profileDraft for work experience fields
    else if (profileDraft.workExperience && Array.isArray(profileDraft.workExperience) && profileDraft.workExperience.length > 0) {
      value = extractWorkFromProfileDraft(profileDraft.workExperience[0], fieldKey) || ''
      if (value) {
        console.log(`💼 ProfileDraft work extraction for ${fieldKey}:`, value)
      }
    }
    // Fallback to original mapResumeDataToFields
    else {
  ...
  // STEP 5: Initialize collections with enhanced sources
  const initialCollections = {}
  ...
  if (schema.collections) {
    schema.collections.forEach(collection => {
      ...
      if (step3Data[collectionKey] && Array.isArray(step3Data[collectionKey])) {
        arrayData = step3Data[collectionKey]
      ...
      initialCollections[collectionKey] = arrayData.length > 0 ? arrayData : [{}]
    })
  }
