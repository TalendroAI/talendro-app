...
const Onb4 = () => {
  ...
  const [employment, setEmployment] = useState([
    { id: 1, company: '', title: '', start: '', end: '', city: '', state: '', reason: '', responsibilities: '', current: false, autoAdded: false }
  ]);
  ...
  useEffect(() => {
    ...
    if (prefill?.step4) {
      ...
      if (step4.workHistory && step4.workHistory.length > 0) {
      ...
      // Populate licenses
      if (step4.licenses && step4.licenses.length > 0) {
      ...
      if (step4.certifications && step4.certifications.length > 0) {
