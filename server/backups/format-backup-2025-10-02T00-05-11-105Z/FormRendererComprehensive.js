      ...
      schema.collections.forEach(collection => {
        ...
        try {
          ...
          if (savedData) {
            ...
            // Try to get collection data from resume parsing
            let collectionData = null
            if (resumeData?.data?.prefill?.step3?.[collection.key]) {
              collectionData = resumeData.data.prefill.step3[collection.key]
            } else if (resumeData?.prefill?.step3?.[collection.key]) {
              collectionData = resumeData.prefill.step3[collection.key]
            } else if (resumeData?.[collection.key]) {
              collectionData = resumeData[collection.key]
            }
            
            if (collectionData && Array.isArray(collectionData) && collectionData.length > 0) {
              initialCollections[collection.key] = collectionData
              console.log(`🔍 FormRendererComprehensive - Loaded ${collection.key} from resume:`, collectionData)
            } else {
