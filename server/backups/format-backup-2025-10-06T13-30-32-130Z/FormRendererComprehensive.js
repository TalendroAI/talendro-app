...
useEffect(() => {
  ...
  // STEP 4: Hydrate each field using schema-driven approach
  const initialForm = {}
  ...
  fields.forEach(field => {
    ...
    // Fallback to mapping helpers for education fields from arrays
    else {
      value = mapEducationFromArray(step3Data, fieldKey) || ''
      if (value) {
        console.log(`🎓 Education array extraction for ${fieldKey}:`, value)
      }
    }
  ...
  if (schema.collections) {
    schema.collections.forEach(collection => {
      ...
      const arrayData = step3Data[collectionKey]
      
      if (Array.isArray(arrayData) && arrayData.length > 0) {
        initialCollections[collectionKey] = arrayData
        console.log(`📚 Collection ${collectionKey} populated:`, arrayData)
      } else {
        initialCollections[collectionKey] = [{}]
...
}, [schema, resumeData]) // React to schema or resumeData changes
