...
export default function Onb2() {
  ...
  const parseAndGo = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ Resume data saved, navigating to step-2-comprehensive...');

      // Jump immediately to Step 2 Comprehensive (Registration with resume data) after resume upload
      jumped.current = true;
      try { 
        navigate('/app/onboarding/step-2-comprehensive', { replace: true }); 
        console.log('🎯 Navigation successful');
      }
      catch (e) { 
        console.log('⚠️ Navigation failed, using window.location:', e);
        window.location.replace('/app/onboarding/step-2-comprehensive'); 
      }

      // Safety: if Router fails silently, hard jump
      setTimeout(() => {
        if (location.pathname !== '/app/onboarding/step-2-comprehensive') {
          console.log('🔄 Safety navigation triggered');
          window.location.assign('/app/onboarding/step-2-comprehensive');
        }
      }, 100);
    } catch (e) {
