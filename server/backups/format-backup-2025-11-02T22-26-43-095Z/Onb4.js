...
const Onb4 = () => {
  ...
  const mapDegreeType = (degree) => {
    ...
    if (lower.includes('bachelor') || lower.includes('ba') || lower.includes('bs') || lower.includes('b.a') || lower.includes('b.s')) return 'bachelor';
    if (lower.includes('master') || lower.includes('ma') || lower.includes('ms') || lower.includes('mba') || lower.includes('m.a') || lower.includes('m.s')) return 'master';
    if (lower.includes('phd') || lower.includes('ph.d') || lower.includes('doctorate') || lower.includes('doctoral')) return 'doctoral';
    if (lower.includes('associate') || lower.includes('aa') || lower.includes('as') || lower.includes('a.a') || lower.includes('a.s')) return 'associate';
    ...
    if (resumeData?.profileDraft) {
      ...
      if (resumeData.profileDraft.education && Array.isArray(resumeData.profileDraft.education) && resumeData.profileDraft.education.length > 0) {
        const educationData = resumeData.profileDraft.education.map((edu, index) => ({
          id: index + 1,
          institution: edu.institution || edu.area || '',
          degreeType: mapDegreeType(edu.studyType || edu.degree || ''),
          fieldOfStudy: edu.area || edu.field || '',
          graduationDate: formatDateForInput(edu.endDate) || '',
          gpa: edu.score || '',
          honors: edu.courses?.join(', ') || '',
          additionalDetails: edu.summary || '',
          stillEnrolled: false
        }));
  ...
  useEffect(() => {
    ...
    // Auto-add job if under 10 years and all current jobs are filled
    ...
    if (parseFloat(years) < REQUIRED_EMPLOYMENT_YEARS && allFilled && employment.length > 0) {
      // Check if the last job is not current
