...
const Onb3 = () => {
  ...
  useEffect(() => {
    ...
    // Auto-add address if under 7 years and all current addresses are filled
    ...
    if (parseFloat(years) < REQUIRED_RESIDENTIAL_YEARS && allFilled && residences.length > 0) {
      // Check if the last address is not current
          ...
          >
            ...
            {residentialYears < REQUIRED_RESIDENTIAL_YEARS && (
              <span className="ml-2 text-xs">
                ({(REQUIRED_RESIDENTIAL_YEARS - residentialYears).toFixed(1)} years needed)
              </span>
            )}
          </button>
          </div>
