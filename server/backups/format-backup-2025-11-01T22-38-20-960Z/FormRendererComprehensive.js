...
function mapResumeDataToFields(resumeData, fieldKey) {
  ...
  const mappings = {
    ...
    // Re-enabled: map ONLY personal/residential city, never derive from work/education
...
function mapDirectResumeData(resumeData, fieldKey) {
  ...
  const mappings = {
    ...
    // Re-enabled: map ONLY from top-level personal location
...
export default function FormRendererComprehensive({ schema, resumeData, initialData = {}, onSubmit }) {
  ...
  useEffect(() => {
    ...
    const fieldsToForce = ['workArrangement', 'protectedVeteran', 'race', 'profileReview', 'talendroAuthorization', 'streetAddress', 'postalCode', 'county', 'city', 'country']
    ...
    fieldsToForce.forEach(fieldKey => {
      ...
      } else if (fieldKey === 'streetAddress' || fieldKey === 'postalCode' || fieldKey === 'county' || fieldKey === 'city' || fieldKey === 'country') {
        // Text/select fields that need forced detection
        shouldForce = !fieldValue || fieldValue === ''
      }
  ...
  useEffect(() => {
    const revalidateFields = () => {
      ...
      allFields.forEach(field => {
        ...
        // Skip specific fields - they are handled by handleChange function
        if (fieldName === 'workArrangement' || fieldName === 'protectedVeteran' || fieldName === 'race' || 
            fieldName === 'streetAddress' || fieldName === 'postalCode' || fieldName === 'county' || 
            fieldName === 'city' || fieldName === 'country') {
          console.log('🔍 Re-validation: Skipping field in main loop:', fieldName)
          return
        }
